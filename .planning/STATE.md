...
- **D6 (SwiftData staged migration crashes under CloudKit)** — blocks CI going green on `feature/appstore-submission-remediation`. `StressMonitorApp.makePrimaryContainer` (`StressMonitorApp.swift:95-101`) builds the container with `migrationPlan: AppMigrationPlan.self` (V1→V2) plus `cloudKitDatabase: .automatic`; on a fresh store this throws `CoreData Code=134504 "Cannot use staged migration with an unknown model version."` — 3 crash/restarts confirmed on a clean CI runner (run 31508758834, job 93836921111) and locally. `makeRecoveredContainer` masks it by deleting the store and rebuilding **without** any migration plan (its own comment: "D5 = Option A: accept data loss") — that's the actual behavior in production today whenever this triggers. Needs a dedicated investigation (`/gsd-debug` or a planned phase), not another ad hoc CI-side guess — this is app code, not CI config. Related but distinct from the now-fixed `StressMeasurement` CloudKit-optionality bug (commit `2d0fa52`).
